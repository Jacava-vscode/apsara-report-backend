...
class RoleDirectoryService {
  ...
  isAvailable() {
    const clusterName = process.env.ROLE_CLUSTER_NAME || 'roles';
  ...
  async initialize() {
    ...
    if (!this.isAvailable()) {
      const clusterName = process.env.ROLE_CLUSTER_NAME || 'roles';
      console.warn(`⚠️  Role directory cluster (${clusterName}) not available. Admin accounts will fall back to in-browser storage.`);
      return;
    }
    ...
    if (!existing) {
      ...
      const clusterName = process.env.ROLE_CLUSTER_NAME || 'roles';
      console.log(`✅ Default administrator seeded in ${clusterName} cluster`);
    } else {
      const updates = {};
      ...
      if (Object.keys(updates).length > 0) {
        ...
        const clusterName = process.env.ROLE_CLUSTER_NAME || 'roles';
        console.log(`🔄 Default administrator permissions normalized in ${clusterName} cluster`);
      }
    }
